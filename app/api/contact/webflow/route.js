...
export async function POST(request) {
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[webflow contact] beacon webhook', res.status, text)
      return Response.json({ success: false, message: `Webhook ${res.status}` }, { status: 500, headers: corsHeaders })
    }
