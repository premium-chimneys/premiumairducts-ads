...
export default function Services({ city }) {
  useEffect(() => {
    ...
    try {
      (function() {
        var offers = [
          { emoji: '❄️', title: 'Free AC Tune-Up', desc: 'A full AC inspection and safety check — on us.', code: 'acfree25' },
          { emoji: '💨', title: '$69 Off Air Duct Cleaning', desc: '$69 off any whole-home air duct cleaning.', code: 'ducts69off' },
          { emoji: '🔧', title: '15% Off HVAC Repair', desc: '15% off any AC or HVAC repair service.', code: 'hvac15off' }
        ];
        ...
        function closeModal() { overlay.classList.remove('active'); unlockScroll(); }
        ...
        function showReveal(index) {
          ...
          confetti.innerHTML = '';
          var colors = ['#2563eb','#60a5fa','#38bdf8','#c2855a','#d4a574','#22c55e','#fff'];
          for (var i = 0; i < 20; i++) {
        ...
        if (ctaBtn) {
          ctaBtn.addEventListener('click', function() {
            var url = getCalendlyUrl('https://calendly.com/hello-ztry/inspection');
  ...
  return (
    <>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        .sv1{background:#f5f5f7;padding:128px 0;font-family:'Inter Tight',sans-serif;position:relative;overflow:hidden}
        .sv1::before,.sv1::after{content:'';position:absolute;border-radius:50%;pointer-events:none}
        .sv1::before{top:-100px;left:10%;width:900px;height:900px;background:radial-gradient(ellipse,rgba(37,99,235,.25) 0%,rgba(37,99,235,.1) 30%,transparent 60%);animation:sv1F1 5s ease-in-out infinite}
        .sv1::after{bottom:-50px;right:5%;width:800px;height:800px;background:radial-gradient(ellipse,rgba(96,165,250,.22) 0%,rgba(96,165,250,.08) 30%,transparent 60%);animation:sv1F2 7s ease-in-out infinite}
        ...
        .sv1-eyebrow{display:inline-flex;align-items:center;gap:8px;margin-bottom:24px;padding:6px 14px 6px 12px;border-radius:8px;background:linear-gradient(135deg,#0b0f18 0%,#152448 50%,#1f3b6e 100%);border:1px solid rgba(37,99,235,.15);box-shadow:0 0 20px rgba(37,99,235,.2),inset 0 0 12px rgba(37,99,235,.1)}
        .sv1-eyebrow-dot{width:5px;height:5px;border-radius:50%;background:#60a5fa;flex-shrink:0;animation:sv1DP 2s ease-in-out infinite}
        @keyframes sv1DP{0%,100%{box-shadow:0 0 4px rgba(96,165,250,.5)}50%{box-shadow:0 0 12px rgba(96,165,250,.9),0 0 24px rgba(37,99,235,.3)}}
        .sv1-eyebrow-text{font-size:12px;font-weight:600;color:#fff;letter-spacing:.04em}
        .sv1-title{font-size:48px;font-weight:700;color:#121a25;letter-spacing:-.04em;line-height:1.08;margin:0}
        ...
        .sv1-subtitle{font-size:15px;font-weight:400;color:#6f7c94;line-height:1.65;margin:0 0 16px}
        .sv1-header-cta{display:inline-flex;align-items:center;gap:8px;font-family:'Inter Tight',sans-serif;font-size:14px;font-weight:600;color:#e0f0fd;text-decoration:none;padding:11px 22px;border-radius:10px;border:1px solid #2563eb;background:linear-gradient(160deg,#5d9be5,#2563eb 25%,#1e40af 50%,#1d4ed8 72%,#3b82f6);box-shadow:inset 0 1px 0 rgba(155,196,240,.55),inset 0 -1px 0 rgba(0,0,0,.22),0 4px 14px rgba(29,78,216,.4);transition:all .22s ease;position:relative;overflow:hidden;cursor:pointer;text-shadow:0 1px 2px rgba(15,29,80,.35)}
        .sv1-header-cta::before{content:'';position:absolute;top:0;left:-70%;width:40%;height:100%;background:linear-gradient(105deg,transparent 35%,rgba(175,210,255,.35) 50%,transparent 65%);transform:skewX(-12deg);pointer-events:none;transition:left .55s ease}
        .sv1-header-cta:hover{transform:translateY(-2px);box-shadow:inset 0 1px 0 rgba(155,196,240,.55),inset 0 -1px 0 rgba(0,0,0,.22),0 8px 22px rgba(29,78,216,.5);border-color:#3b82f6}
        ...
        .sv1-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:16px}
        ...
        .sv1-card-icon{width:44px;height:44px;border-radius:14px;background:linear-gradient(160deg,#d4854a,#b86028 25%,#8b3c14 50%,#a04c1e 72%,#cc7535);border:1px solid #c06828;display:flex;align-items:center;justify-content:center;flex-shrink:0;position:relative;overflow:hidden;transition:all .3s ease;color:#fff;box-shadow:inset 0 1px 0 rgba(240,175,110,.5),inset 0 -1px 0 rgba(0,0,0,.2),0 2px 8px rgba(140,60,18,.25)}
        ...
        .sv1-card-icon svg{position:relative;z-index:1;filter:drop-shadow(0 1px 2px rgba(15,29,80,.35));transform-origin:center}
        @keyframes sv1IconPulse{0%,100%{transform:scale(1)}50%{transform:scale(1.12)}}
        @keyframes sv1IconSway{0%,100%{transform:rotate(-8deg)}50%{transform:rotate(8deg)}}
        @keyframes sv1IconBreathe{0%,100%{transform:scale(1)}50%{transform:scale(1.08)}}
        @keyframes sv1IconTwinkle{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.55;transform:scale(.92)}}
        @keyframes sv1IconSpin{0%{transform:rotate(0)}100%{transform:rotate(360deg)}}
        @keyframes sv1IconFlicker{0%,100%{transform:scale(1) translateY(0)}25%{transform:scale(1.08) translateY(-1px)}50%{transform:scale(.96) translateY(0)}75%{transform:scale(1.05) translateY(-.5px)}}
        .sv1-grid .sv1-card:nth-child(1) .sv1-card-icon svg{animation:sv1IconPulse 2.4s ease-in-out infinite}
        .sv1-grid .sv1-card:nth-child(2) .sv1-card-icon svg{animation:sv1IconSway 2.2s ease-in-out infinite}
        .sv1-grid .sv1-card:nth-child(3) .sv1-card-icon svg{animation:sv1IconSway 2.6s ease-in-out infinite}
        .sv1-grid .sv1-card:nth-child(4) .sv1-card-icon svg{animation:sv1IconBreathe 2.8s ease-in-out infinite}
        .sv1-grid .sv1-card:nth-child(5) .sv1-card-icon svg{animation:sv1IconPulse 2.4s ease-in-out infinite}
        .sv1-grid .sv1-card:nth-child(6) .sv1-card-icon svg{animation:sv1IconTwinkle 2s ease-in-out infinite}
        .sv1-grid .sv1-card:nth-child(7) .sv1-card-icon svg{animation:sv1IconSway 2.4s ease-in-out infinite}
        .sv1-grid .sv1-card:nth-child(8) .sv1-card-icon svg{animation:sv1IconSpin 8s linear infinite}
        .sv1-grid .sv1-card:nth-child(9) .sv1-card-icon svg{animation:sv1IconFlicker 1.6s ease-in-out infinite}
        ...
        /* CTA BANNER */
        .sv1-cta{margin-top:16px;border-radius:20px;background:linear-gradient(135deg,#0b0f18,#101a30 50%,#0b0f18);border:1px solid rgba(37,99,235,.2);display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:0;position:relative;overflow:hidden;transition:all .3s ease}
        .sv1-cta:hover{border-color:rgba(37,99,235,.4);box-shadow:0 16px 48px rgba(37,99,235,.12),0 0 80px rgba(37,99,235,.04)}
        .sv1-cta::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#2563eb,#60a5fa,#38bdf8,#60a5fa,#2563eb,transparent);background-size:300% 100%;animation:sv1CA 4s ease-in-out infinite;z-index:2}
        @keyframes sv1CA{0%{background-position:0% 50%}50%{background-position:100% 50%}100%{background-position:0% 50%}}
        .sv1-cta::after{content:'';position:absolute;left:40px;top:50%;transform:translateY(-50%);width:120px;height:120px;background:radial-gradient(circle,rgba(37,99,235,.2) 0%,transparent 70%);pointer-events:none;z-index:0;animation:sv1CG 3s ease-in-out infinite}
        ...
        .sv1-cta-badge-label{font-family:'Inter Tight',sans-serif;font-size:12px;font-weight:700;color:#60a5fa;letter-spacing:.02em;margin-bottom:4px}
        .sv1-cta-badge-amount{font-family:'Inter Tight',sans-serif;font-size:40px;font-weight:800;letter-spacing:-.03em;line-height:1;background:linear-gradient(180deg,#fff 30%,#60a5fa 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
        ...
        .sv1-cta-btn{display:inline-flex;align-items:center;gap:8px;font-family:'Inter Tight',sans-serif;font-size:15px;font-weight:600;color:#e0f0fd;padding:14px 32px;border-radius:12px;border:1px solid #2563eb;background:linear-gradient(160deg,#5d9be5,#2563eb 25%,#1e40af 50%,#1d4ed8 72%,#3b82f6);box-shadow:inset 0 1px 0 rgba(155,196,240,.55),inset 0 -1px 0 rgba(0,0,0,.22),0 4px 20px rgba(29,78,216,.5);transition:all .22s ease;position:relative;overflow:hidden;cursor:pointer;text-shadow:0 1px 2px rgba(15,29,80,.35);text-decoration:none;white-space:nowrap;width:100%;justify-content:center}
        .sv1-cta-btn::before{content:'';position:absolute;top:0;left:-70%;width:40%;height:100%;background:linear-gradient(105deg,transparent 35%,rgba(175,210,255,.35) 50%,transparent 65%);transform:skewX(-12deg);pointer-events:none;transition:left .55s ease}
        .sv1-cta-btn:hover{transform:translateY(-2px);box-shadow:inset 0 1px 0 rgba(155,196,240,.55),inset 0 -1px 0 rgba(0,0,0,.22),0 8px 28px rgba(29,78,216,.6);border-color:#3b82f6}
        ...
        .sv1-gift-modal{background:rgba(255,255,255,.97);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border:1px solid rgba(37,99,235,.12);border-radius:24px;width:90%;max-width:520px;padding:44px 40px;position:relative;overflow:hidden;text-align:center;animation:sv1GS .4s cubic-bezier(.16,1,.3,1);box-shadow:0 32px 64px rgba(0,0,0,.2),0 0 100px rgba(37,99,235,.06)}
        @keyframes sv1GS{0%{opacity:0;transform:translateY(30px) scale(.95)}100%{opacity:1;transform:translateY(0) scale(1)}}
        .sv1-gift-modal::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#2563eb,#60a5fa,#38bdf8,#60a5fa,#2563eb,transparent);background-size:300% 100%;animation:sv1CA 4s ease-in-out infinite;z-index:2}
        .sv1-gift-close{position:absolute;top:16px;right:16px;width:32px;height:32px;border:none;background:rgba(0,0,0,.04);border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:18px;color:#6f7c94;transition:all .2s ease;z-index:3}
        .sv1-gift-close:hover{background:rgba(0,0,0,.08);color:#121a25}
        .sv1-gift-title{font-family:'Inter Tight',sans-serif;font-size:24px;font-weight:800;color:#121a25;margin:0 0 8px;letter-spacing:-.03em}
        .sv1-gift-subtitle{font-family:'Inter Tight',sans-serif;font-size:14px;color:#6f7c94;margin:0 0 32px;line-height:1.5}
        .sv1-gift-boxes{display:flex;gap:16px;justify-content:center;margin-bottom:24px}
        .sv1-gift-box{width:140px;height:160px;border-radius:16px;background:#f6f8fd;border:1.5px solid rgba(37,99,235,.1);display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;transition:all .3s cubic-bezier(.16,1,.3,1);position:relative;overflow:hidden}
        .sv1-gift-box::before{content:'';position:absolute;inset:-1px;border-radius:16px;background:conic-gradient(from 0deg,rgba(37,99,235,.4),transparent 30%,transparent 70%,rgba(96,165,250,.3));animation:sv1Spin 4s linear infinite;z-index:0;opacity:0;transition:opacity .3s ease}
        .sv1-gift-box::after{content:'';position:absolute;inset:1px;border-radius:15px;background:linear-gradient(160deg,#f6f8fd,#ecf0fa);z-index:0}
        @keyframes sv1Spin{0%{transform:rotate(0)}100%{transform:rotate(360deg)}}
        .sv1-gift-box:hover::before{opacity:1}
        .sv1-gift-box:hover{transform:translateY(-6px) scale(1.04);border-color:rgba(37,99,235,.25);box-shadow:0 12px 32px rgba(37,99,235,.12)}
        ...
        .sv1-gift-box-label{font-family:'Inter Tight',sans-serif;font-size:12px;font-weight:600;color:#6f7c94;margin-top:12px;position:relative;z-index:1}
        ...
        .sv1-gift-box.shuffling.chosen{animation:sv1GBc .3s ease-in-out infinite alternate;border-color:rgba(37,99,235,.3);box-shadow:0 0 30px rgba(37,99,235,.1)}
        ...
        .sv1-gift-reveal-desc{font-family:'Inter Tight',sans-serif;font-size:15px;color:#6f7c94;margin:0 0 24px;animation:sv1RI .5s ease-out .4s both}
        ...
        .sv1-gift-code{font-family:'Inter Tight',monospace;font-size:20px;font-weight:800;color:#121a25;letter-spacing:.08em;padding:12px 24px;background:#f6f8fd;border:2px dashed rgba(37,99,235,.2);border-radius:10px}
        .sv1-gift-copy{font-family:'Inter Tight',sans-serif;font-size:13px;font-weight:600;color:#2563eb;background:rgba(37,99,235,.06);border:1px solid rgba(37,99,235,.15);border-radius:8px;padding:12px 18px;cursor:pointer;transition:all .2s ease}
        .sv1-gift-copy:hover{background:rgba(37,99,235,.12);border-color:rgba(37,99,235,.3)}
        .sv1-gift-copy.copied{color:#16a34a;border-color:rgba(34,197,94,.2);background:rgba(34,197,94,.06)}
        .sv1-gift-redeem{font-family:'Inter Tight',sans-serif;font-size:13px;color:#99a0b2;margin:0;line-height:1.5;animation:sv1RI .5s ease-out .6s both}
        ...
        <div className="sv1-inner">
          <div className="sv1-header">
            <div className="sv1-header-left">
              <h2 className="sv1-title">The <span className="sv1-title-gradient">gold standard</span> in<br />the HVAC industry</h2>
            </div>
            <div className="sv1-header-right">
              <p className="sv1-subtitle">Expert repairs and cleaning for your home. Professional service, every day.</p>
          ...
          <div className="sv1-grid">
            <a href={`/${city.slug}/ac-repair`} className="sv1-card"><div className="sv1-card-img-wrap"><img className="sv1-card-img" src="/images/ac-repair.jpg" alt="AC Repair" /></div><div className="sv1-card-body"><div className="sv1-card-header"><div className="sv1-card-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="2" x2="12" y2="22" /><line x1="2" y1="12" x2="22" y2="12" /><line x1="4.93" y1="4.93" x2="19.07" y2="19.07" /><line x1="19.07" y1="4.93" x2="4.93" y2="19.07" /></svg></div><h3 className="sv1-card-title">AC Repair</h3></div><p className="sv1-card-desc">Fast diagnostics and precise repairs on any AC unit, any brand. We get your system running before the heat wins.</p><span className="sv1-card-link">Learn More <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M5 12h14M13 5l7 7-7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg></span></div></a>
            <a href={`/${city.slug}/air-duct-cleaning`} className="sv1-card"><div className="sv1-card-img-wrap"><img className="sv1-card-img" src="/images/air-duct-cleaning.jpg" alt="Air Duct Cleaning" /></div><div className="sv1-card-body"><div className="sv1-card-header"><div className="sv1-card-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.7 7.7a2.5 2.5 0 1 1 1.8 4.3H2" /><path d="M9.6 4.6A2 2 0 1 1 11 8H2" /><path d="M12.6 19.4A2 2 0 1 0 14 16H2" /></svg></div><h3 className="sv1-card-title">Air Duct Cleaning</h3></div><p className="sv1-card-desc">What&apos;s hiding in your ducts is what your family&apos;s breathing. Deep extraction, before &amp; after photos, clean results.</p><span className="sv1-card-link">Learn More <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M5 12h14M13 5l7 7-7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg></span></div></a>
            <a href={`/${city.slug}/dryer-vent-cleaning`} className="sv1-card"><div className="sv1-card-img-wrap"><img className="sv1-card-img" src="/images/dryer-vent-cleaning.jpg" alt="Dryer Vent Cleaning" /></div><div className="sv1-card-body"><div className="sv1-card-header"><div className="sv1-card-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.72-3.45-.87-6.65 2-9 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z" /></svg></div><h3 className="sv1-card-title">Dryer Vent Cleaning</h3></div><p className="sv1-card-desc">The #1 overlooked fire hazard in the home. We clear the blockage completely and restore safe, full airflow.</p><span className="sv1-card-link">Learn More <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M5 12h14M13 5l7 7-7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg></span></div></a>
            <a href={`/${city.slug}/hvac-repair`} className="sv1-card"><div className="sv1-card-img-wrap"><img className="sv1-card-img" src="/images/hvac-repair.jpg" alt="HVAC Repair" /></div><div className="sv1-card-body"><div className="sv1-card-header"><div className="sv1-card-icon"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 14.76V3.5a2.5 2.5 0 0 0-5 0v11.26a4.5 4.5 0 1 0 5 0z" /></svg></div><h3 className="sv1-card-title">HVAC Repair</h3></div><p className="sv1-card-desc">One call. Full system. Diagnostics, part replacement, and tune-up — we don&apos;t leave until everything runs right.</p><span className="sv1-card-link">Learn More <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M5 12h14M13 5l7 7-7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg></span></div></a>
          </div>
          ...
          <div className="sv1-cta">
            ...
            <div className="sv1-cta-content">
              ...
              <p className="sv1-cta-desc">Our certified technicians are available this week. Book now and get a full HVAC system assessment.</p>
